...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text("◾◾◾◾** Now Uploading To Google Cloud...**")
    ...
    if os.path.isfile(file_upload):
        ...
        LOGGER.info(gjay)
        button = []
        button.append([pyrogram.InlineKeyboardButton(text="◾🦊 GDrive URL 🦊◾", url=f"{gau_link}")])
        if INDEX_LINK:
            indexurl = f"{INDEX_LINK}/{file_upload}"
            tam_link = requests.utils.requote_uri(indexurl)
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🦊◾◾ : ` {file_upload} ` **Has Uploaded successfully To Google Cloud **<a href='tg://user?id={g_id}'>🦊◾◾◾🦊</a>\n**📼 File Size **: {gjay}", reply_markup=button_markup)
        #await message.edit_text(f"""🦊◾◾ : ` {file_upload}` **Has Uploaded successfully To Google Cloud 🦊◾◾◾🦊\n\n☁️ Cloud URL:  <a href="{gau_link}">FileLink</a>\nℹ️ Direct URL:  <a href="{tam_link}">IndexLink</a>""")
        ...
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f" **🦊◾ : Folder Has Uploaded Successfully To** `{tt} `** Google Cloud**\n <a href='tg://user?id={g_id}'>🦊◾◾◾🦊</a>\n\📼 **Folder Size **: {gjay}", reply_markup=button_markup)
        #await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
